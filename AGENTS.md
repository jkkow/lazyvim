...
The Lua Language Server (`lua_ls`) is the primary linter used within Neovim. Diagnostics and workspace settings are configured via `.luarc.json` (disables `undefined-global` for Neovim's `vim` global) and `.neoconf.json`.
...
- **Opencode**: This repository actively utilizes `opencode.nvim`. Configuration is located at `lua/plugins/opencode.lua`.
- **Skills**: Agents operating within this repository should leverage available Opencode skills (e.g., `test_skill`, `modern-git-commit`, `make-skill`) via the skill tool when workflows match those specific domains.
